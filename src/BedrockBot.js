...
class BedrockBot extends EventEmitter {
    ...
    async start() {
        ...
        try {
            ...
            if (autoReconnect) {
                ...
                return; // Don't throw error, let AutoReconnect handle it
            }
    ...
    async handleSpawn(packet) {
        ...
        this.hasEmittedDisconnect = false; // Reset the flag on successful connection
    ...
    handleEnd(packet) {
        ...
        // Don't emit disconnect here since it's redundant
    }
    ...
    handleError(error) {
        ...
        // Only emit disconnect for initial connection errors
        // (not for errors that happen after disconnect/end events)
    ...
    async reinitializeComponents() {
        try {
            // Check if AutoReconnect component exists (first time or after manual stop)
            ...
            // Create non-AutoReconnect components
            const components = [
                new Advertisement(this, this.config, this.logger),
                new Teleport(this, this.config, this.logger)
            ];
    ...
    async performReconnect() {
        try {
            ...
            // The AutoReconnect component will handle further retry attempts
            throw error;
        }
    }
